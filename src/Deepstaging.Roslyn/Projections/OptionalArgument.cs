...
/// <summary>
/// An optional attribute argument that may or may not contain a value.
/// Provides a fluent API for extracting and transforming attribute argument values.
/// </summary>
/// <typeparam name="TSource">The source type of the argument.</typeparam>
public readonly struct OptionalArgument<TSource> : IProjection<TSource?>, IEquatable<OptionalArgument<TSource>>
{
    private readonly TSource? _value;
    private readonly bool _hasValue;

    private OptionalArgument(TSource? value, bool hasValue)
    {
        _value = value;
        _hasValue = hasValue;
    }
    ...
    /// </summary>
    public static OptionalArgument<TSource> WithValue(TSource value) => new(value, true);

    /// <summary>
    /// Creates a projection without a value.
    /// </summary>
    public static OptionalArgument<TSource> Empty() => new(default, false);
    ...
    public OptionalArgument<TResult> Map<TResult>(Func<TSource, TResult> mapper)
    {
        if (_hasValue && _value != null)
            return OptionalArgument<TResult>.WithValue(mapper(_value));

        return OptionalArgument<TResult>.Empty();
    }
    ...
    /// <returns>A new projected argument with the result type.</returns>
    public OptionalArgument<TResult> Select<TResult>(Func<TSource, TResult> selector) => Map(selector);
    ...
    /// <returns>A new projected argument with the enum type.</returns>
    public OptionalArgument<TEnum> ToEnum<TEnum>() where TEnum : struct, Enum
    {
        if (!_hasValue || _value == null)
        ...
        if (_value is string strValue && Enum.TryParse<TEnum>(strValue, out var enumValue))
            return OptionalArgument<TEnum>.WithValue(enumValue);
    ...
    public TSource OrThrow(string? message = null) => _hasValue && _value != null
        ? _value
        : throw new InvalidOperationException(message ?? "Attribute argument not found");
    ...
    /// <returns>This projected argument for chaining.</returns>
    public OptionalArgument<TSource> Do(Action<TSource> action)
    {
        if (_hasValue && _value != null)
            action(_value);
    ...
    /// <param name="action">The action to execute.</param>
    /// <returns>This projected argument for chaining.</returns>
    public OptionalArgument<TSource> OrElse(Action action)
    {
        if (!_hasValue)
            action();
    ...
    /// <summary>
    /// Attempts to get the argument value. Returns true if value is present.
    ...
    /// </summary>
    public static bool operator ==(OptionalArgument<TSource> left, TSource? right) => left.Equals(right);
    ...
    /// <summary>
    /// Determines whether the current instance equals another OptionalArgument instance.
    /// </summary>
    public bool Equals(OptionalArgument<TSource> other) =>
        EqualityComparer<TSource?>.Default.Equals(_value, other._value);
}