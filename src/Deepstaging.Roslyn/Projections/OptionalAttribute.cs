    ...
    : IValidatableProjection<AttributeData, ValidAttribute>
{
    ...
    /// </summary>
    public static OptionalAttribute WithValue(AttributeData attribute) => new(attribute);

    /// <summary>
    /// Creates a projection without an attribute.
    /// </summary>
    public static OptionalAttribute Empty() => new(null);

    /// <summary>
    /// Creates a projection from a nullable attribute.
    /// </summary>
    public static OptionalAttribute FromNullable(AttributeData? attribute) =>
        attribute != null ? WithValue(attribute) : Empty();
    ...
    public OptionalArgument<T> ConstructorArg<T>(int index) =>
        attribute?.GetConstructorArgument<T>(index) ?? OptionalArgument<T>.Empty();
    ...
    public OptionalArgument<T> NamedArg<T>(string name) =>
        attribute?.GetNamedArgument<T>(name) ?? OptionalArgument<T>.Empty();
    ...
    public OptionalValue<TResult> Map<TResult>(Func<ValidAttribute, TResult> mapper) => attribute != null
    ...
    public OptionalValue<TResult> WithArgs<TResult>(Func<ValidAttribute, TResult> extractor) => Map(extractor);
    ...
    public ImmutableArray<ValidSymbol<INamedTypeSymbol>> GetTypeArguments() =>
        attribute?.AttributeClass is { IsGenericType: true } attrType
    ...
    /// </summary>
    public AttributeData? OrNull() => attribute;
    ...
    public AttributeData OrThrow(string? message = null) =>
        attribute ?? throw new InvalidOperationException(message ?? "Attribute not found");
    ...
    public AttributeData OrThrow(Func<string> messageFactory) =>
        attribute ?? throw new InvalidOperationException(messageFactory());
    ...
    public AttributeData OrThrow(Func<Exception> exceptionFactory) => attribute ?? throw exceptionFactory();

    /// <summary>
    /// Validates this optional attribute to an Attribute with guaranteed non-null access.
    /// Returns null if the attribute is not present.
    /// </summary>
    public ValidAttribute? Validate() => attribute != null ? ValidAttribute.From(attribute) : null;

    /// <summary>
    /// Validates this optional attribute, throwing if not present.
    /// </summary>
    public ValidAttribute ValidateOrThrow(string? message = null) => attribute != null
    ...
    /// </summary>
    public bool IsNotValid(out ValidAttribute validated) => !TryValidate(out validated);
    ...
    public TResult Match<TResult>(
        ...
        Func<TResult> whenEmpty) => attribute != null ? whenPresent(attribute) : whenEmpty();
    ...
    /// </summary>
    public AttributeData OrDefault(AttributeData defaultValue) => attribute ?? defaultValue;
