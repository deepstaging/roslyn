...
public static class ManagedPropsFileExtensions
{
    ...
    public static CodeAction ModifyPropsFileAction<TProps>(this Project project, string title, Action<XDocument> modify)
        ...
        project.ModifyPropsFileAction<TProps>(title, directory: null, modify);
    ...
    /// <param name="modify">Action to modify the XML document after defaults are ensured.</param>
    public static CodeAction ModifyPropsFileAction<TProps>(this Project project, string title, string? directory, Action<XDocument> modify)
        where TProps : ManagedPropsFile, new()
    {
    ...
    public static ProjectFileActionsBuilder ModifyPropsFile<TProps>(this ProjectFileActionsBuilder builder, Action<XDocument> modify)
        where TProps : ManagedPropsFile, new() =>
        builder.ModifyPropsFile<TProps>(directory: null, modify);
    ...
    /// <param name="directory">Optional directory relative to the project root (e.g., <c>.config</c>).</param>
    /// <param name="modify">Action to modify the XML document after defaults are ensured.</param>
    public static ProjectFileActionsBuilder ModifyPropsFile<TProps>(this ProjectFileActionsBuilder builder, string? directory, Action<XDocument> modify)
        where TProps : ManagedPropsFile, new()
    {
        var props = new TProps();
